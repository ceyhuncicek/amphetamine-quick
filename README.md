...
Store screenshots go in `metadata/` at 2000×1250. The easiest way to get them at
the right size: with `npm run dev` running, open one of the commands in Raycast,
then run the built-in **Window Capture** command (search "Window Capture") and pick
"Save to Metadata", which writes the file straight into `metadata/`. Add one to
three showing Start Session with the argument boxes.

Then:

```sh
npm run lint     # must be clean (author must be a registered Raycast username)
npm run build
npm run publish  # forks and opens the store PR
```
